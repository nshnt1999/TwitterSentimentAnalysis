...
consumerKey = 'DHev453NkqyigmHCKCmp9Fd4z'
consumerSecret = '3yuQ3hLKFZVg6spKPAwhq3dS3Djvc9qqkUjm41OTf71qaRfIkd'
accessToken = '1335543120159625216-HIqvLRrSpxU9YsrGQjK58X7W69cvBE'
accessTokenSecret = 'SWzVJBxvnHRMmFFQZVzk3jFJuCZ6EccUjBqvCJJAjs1zs'
...
def app():
	...
	activities=["Generate Twitter Data","Tweet Analysis"]

	choice = st.sidebar.selectbox("Choose Your Activity",activities)

	

	if choice=="Tweet Analysis":

		st.subheader("Analyze the tweets of your favourite Personalities")

		st.subheader("This tool performs the following tasks :")

		st.write("1. Gets the 5 most recent tweets from the given twitter handel")
		...
		raw_text = st.text_area("Enter the twitter handle of the Person(without @)")



		st.markdown("<--------     Also Do checkout the another tool in the sidebar")
		...
		st.subheader("This tool collects the last 100 tweets from the twitter handel & Performs the following tasks")
		...
		st.markdown("<--------     Also Do checkout the another tool from the sidebar")
		...
		def get_data(user_name):
			...
			def cleanTxt(text):
				...
				text = re.sub('RT[\s]+', '', text) # Removing RT
				text = re.sub('https?:\/\/\S+', '', text) # Removing hyperlink
				text = ' '.join([txt for txt in text.split() if len(txt)>2])
				return text
	...
	st.subheader(' ------------------------Created By :  Nishant'+emoji.emojize(':grinning_face_with_big_eyes:')+'----------------------' )
